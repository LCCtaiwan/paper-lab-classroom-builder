...
const assets = resolve(String(args.assets || `public/courses/${manifest.courseId}`));
if (!existsSync(assets)) throw new Error(`找不到課程 assets：${assets}`);
const output = resolve(String(args.out || `../output/${manifest.courseId}-paper-lab.zip`));
...
try {
  ...
  cpSync(resolve("../docs/CLOUDFLARE.md"), join(staging, "DEPLOY.md"));
