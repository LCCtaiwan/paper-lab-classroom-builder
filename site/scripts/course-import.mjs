...
import { defaultTitle, ensureParent, inspectPdf, parseArgs, renderPdfAssets, resolveArgPath } from "./course-utils.mjs";

const args = parseArgs(process.argv.slice(2));
const pdf = resolveArgPath(args.pdf, "../content/handout.pdf");
const inspected = inspectPdf(pdf);
const courseId = safeCourseId(args.id || defaultTitle(pdf));
const output = resolveArgPath(args.out, "../course.manifest.json");
const assets = resolveArgPath(args.assets, `public/courses/${courseId}`);
